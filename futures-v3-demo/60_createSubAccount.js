 ...
 *   1. childSignature  - Sub-account signs message body (excluding childSignature) using EIP-712
 *   2. signature       - Master account signs full message body (including childSignature) using MASTER wallet key
 */
...
const PARAMS = {
    ...
    subAccountName: 'trading-desk-1',            // 子账户名称 / Sub-account name
    // childPrivateKey: '0x...',                 // 子账户私钥（仅用于生成 childSignature）/ Sub-account private key
};
...
async function createSubAccount() {
    try {
        console.log('Request / 请求:', 'POST /fapi/v3/createSubAccount');

        const { getNonce, buildQueryString } = require('./utils');
        const nonce = String(getNonce());

        // ── Step 1: Sub-account signature / 子账户签名 ──
        // Message body: subAccountName + subSourceAddr + nonce + user + signer (no childSignature)
        const childMsgBody = `subAccountName=${PARAMS.subAccountName}&subSourceAddr=${PARAMS.subSourceAddr}&nonce=${nonce}&user=${config.USER_ADDRESS}&signer=${config.SIGNER_ADDRESS}`;
        ...
        const response = await axios.post(
            `${config.BASE_URL}/fapi/v3/createSubAccount`,
            queryString,
